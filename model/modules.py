...
class BarEncoder_ChordSeq(nn.Module):
    ...
    def __init__(self):
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(16, 16, (2, 1), stride=(2, 1)),  ###########2,1
            nn.BatchNorm2d(16),
            nn.LeakyReLU(0.2, True)
        )
        self.layer3 = nn.Sequential(
    ...
    def forward(self, condition):
        # condition shape: (?, 1, 48, 84)32
        h0 = self.layer0(condition) # (?, 16, 48, 7) 32
        h1 = self.layer1(h0)  # (?, 16, 48, 1) 23
...
class BarGenerator_ChordSeq(nn.Module):
    def __init__(self, z_dim):
        ...
        self.layer4 = nn.Sequential(
            nn.ConvTranspose2d(272, 128, (2, 1), stride=(2, 1), padding=0),  #########2,1
            nn.BatchNorm2d(128),
            nn.ReLU(True),
        )
...
class BarDiscriminator_ChordSeq(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(6, 128, (1, 12), stride=(1, 12)),  # (?, 128, 48, 7)  32
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 128, (1, 7), stride=(1, 7)),  # (?, 128, 48, 1)  32
            nn.LeakyReLU(0.2, True),
            nn.Conv2d(128, 128, (2, 1), stride=(2, 1)),  # (?, 128, 24, 1)  16
            nn.LeakyReLU(0.2, True),
