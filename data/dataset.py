...
class LSADataset(torch.utils.data.Dataset):
    def __init__(self, path):
        ...
        self.data_X = np.load(os.path.join(path, 'all_chord_X.npy'))
        self.data_y = np.load(os.path.join(path, 'all_chord_chroma_Y7.npy'))
...
def get_my_data(BATCH_SIZE=64, trainer_type=0):
    ...
    t_path = 'data/chord_sequence/tra'
    v_path = 'data/chord_sequence/val'

    if trainer_type == 1:  # chroma sequence
        t_path = 'data/chroma_sequence/tra'
        v_path = 'data/chroma_sequence/val'
